...
class nnUNetPredictor(object):
    def __init__(self,
                 ...
                 bottleneck_features = False, 
                 average_folds_patches_mirrors = (True,False,True), 
                 bottleneck_features_ext = 'npz'):
        ...
        self.save_bottleneck_features = bottleneck_features
        if self.save_bottleneck_features:
            self.slicers = []
            self.bottleneck_features = []
            self.average_folds_patches_mirrors = average_folds_patches_mirrors
            self.bottleneck_features_ext = bottleneck_features_ext

    def register_bottleneck_hook(self):
        def hook(module, input, output):
            self.bottleneck_features.append(output)
            #print("Feats",self.bottleneck_features[-1].shape, len(self.bottleneck_features))

        bottleneck_layer = self.network.encoder.stages[-1]
        bottleneck_layer.register_forward_hook(hook)

    def get_bottleneck_features(self):
        return self.bottleneck_features
    
    def save_bottleneck_features_as_array(self,
                                            ofile:str, extension='npz',
                                            average_mirrored_patches=True,
                                            average_patches=False,
                                            average_folds=True):
        original_shape = self.bottleneck_features[0].shape  # Exclude batch dimension
        stacked_bottleneck = torch.stack(self.bottleneck_features)
        #stacked as (Folds, patches_per_images, mirrors_per_patch, *original_shape) shape
        stacked_bottleneck = stacked_bottleneck.view(
            len(self.list_of_parameters),
            len(self.slicers[0]), # the slicers is a list of list with the slices ranges per patch
            self.predictions_per_patch_due_to_mirrroing,
            *original_shape
        )
        # sanity check in case there are changes in torch.stack or view methods between versions
        for fold_idx in range(len(self.list_of_parameters)):
            for patch_idx in range(len(self.slicers[0])):
                for mirror_idx in range(self.predictions_per_patch_due_to_mirrroing):
                    # Get original tensor position in list
                    list_idx = fold_idx * (len(self.slicers[0]) * self.predictions_per_patch_due_to_mirrroing) + patch_idx * self.predictions_per_patch_due_to_mirrroing + mirror_idx
                    # Compare
                    from_list = self.bottleneck_features[list_idx]
                    from_stacked = stacked_bottleneck[fold_idx, patch_idx, mirror_idx]
                    # Check if identical
                    assert torch.all(from_list == from_stacked), "Bottleneck features are not properly stacked"

        # Average over the specified axes
        if average_mirrored_patches:
            stacked_bottleneck = stacked_bottleneck.mean(dim=2)
        if average_patches:
            stacked_bottleneck = stacked_bottleneck.mean(dim=1)
        if average_folds:
            stacked_bottleneck = stacked_bottleneck.mean(dim=0)
        #print("Averaged bottleneck features shape:", stacked_bottleneck.shape)

        # Save or process the averaged bottleneck features as needed
        ofile = ofile+'_bottleneck-feats.'+extension if ofile is not None else None
        if ofile.endswith('.npz'):
            np.savez_compressed(ofile, stacked_bottleneck.cpu().numpy())
        elif ofile.endswith('.npy'):
            np.save(ofile, stacked_bottleneck.cpu().numpy())
        elif ofile.endswith('.pt'):
            torch.save(stacked_bottleneck, ofile)
        else:
            raise ValueError("Unsupported file extension. Please use '.npz', '.npy', or '.pt'.")
    ...
    def initialize_from_trained_model_folder(self, model_training_output_dir: str,
        ...
        mirroring = self.allowed_mirroring_axes if self.use_mirroring else None
        self.predictions_per_patch_due_to_mirrroing = 1 if mirroring is None else 2 ** len(mirroring) 
        if self.save_bottleneck_features:
            print("Registering hook for bottleneck features...")
            self.register_bottleneck_hook()
    ...
    def predict_from_data_iterator(self,
        ...
        with multiprocessing.get_context("spawn").Pool(num_processes_segmentation_export) as export_pool:
            ...
            for preprocessed in data_iterator:
                ...
                properties = preprocessed['data_properties'] # changed from data_properities to data_properties
                ...
                if self.save_bottleneck_features:
                    av_folds, av_patches, av_mirrors = self.average_folds_patches_mirrors
                    self.save_bottleneck_features_as_array(
                        ofile=ofile, 
                        average_folds=av_folds, 
                        average_mirrored_patches=av_mirrors, 
                        average_patches=av_patches, 
                        extension=self.bottleneck_features_ext
                    )
                    self.bottleneck_features = []
                    self.slicers = []
    ...
    def predict_sliding_window_return_logits(self, input_image: torch.Tensor) \
        ...
        with torch.no_grad():
            with torch.autocast(self.device.type, enabled=True) if self.device.type == 'cuda' else dummy_context():
                ...
                # Info for the patches essential for bottleneck features
                if self.save_bottleneck_features:
                    self.slicers.append(slicers)
...
def predict_entry_point():
    ...
    parser.add_argument('-device', type=str, default='cuda', required=False,
                        help="Use this to set the device the inference should run with. Available options are 'cuda' "
                             "(GPU), 'cpu' (CPU) and 'mps' (Apple M1/M2). Do NOT use this to set which GPU ID! "
                             "Use CUDA_VISIBLE_DEVICES=X nnUNetv2_predict [...] instead!")
        ## Added for bottleneck features
    parser.add_argument('--bottleneck_features', action='store_true', required=False, default=False,
                        help='Set this flag to obtain per patch bottleneck features')
    parser.add_argument('--average_folds_patches_mirrors',
                        type=lambda x: bool(int(x)) if x.isdigit() else x.lower() == 'true',
                        nargs=3,
                        default=[True, False, True],
                        help='Set this to average the bottleneck features over folds, patches and mirrors. '
                             'Provide three booleans separated , e.g., "1 0 1" or "true false True" for (True False True) '
                             'Default: (True, False, True)')
    parser.add_argument('--bottleneck_features_ext', type=str, required=False, default='npz', choices=['npz', 'npy', 'pt'],
                        help='Extension for bottleneck features. Default: npy')
    ...
    predictor = nnUNetPredictor(tile_step_size=args.step_size,
                                ...
                                bottleneck_features=args.bottleneck_features, 
                                average_folds_patches_mirrors=args.average_folds_patches_mirrors,
                                bottleneck_features_ext=args.bottleneck_features_ext)
